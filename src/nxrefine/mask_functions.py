...
from nexusformat.nexus import NXcollection, NXdata, NXentry, NXfield, NXprocess
...
def print_peak(input_entry, index):
    print(
        np.around(
            np.array(
                [input_entry.peaks_inferred.x[index].nxdata, input_entry.
                 peaks_inferred.y[index].nxdata, input_entry.peaks_inferred.
                 z[index].nxdata]),
            decimals=3))
    print(input_entry.peaks_inferred.H[index],
          input_entry.peaks_inferred.K[index],
          input_entry.peaks_inferred.L[index])
    print(
        input_entry.peaks_inferred.pixel_count[index],
        input_entry.peaks_inferred.frame_intensity[index],
        input_entry.peaks_inferred.radius[index])
...
def get_xyz_mask(input_entry, H, K, L, verbose=False):
    ...
    chi_mat = np.array(
        [[1, 0, 0], [0, np.cos(chi), -np.sin(chi)],
         [0, np.sin(chi), np.cos(chi)]])
    omega_mat = np.array([[np.cos(omega), -np.sin(omega), 0],
                         [np.sin(omega), np.cos(omega), 0], [0, 0, 1]])
    gonpitch_mat = np.array([[np.cos(gonpitch), 0, np.sin(gonpitch)],
                             [0, 1, 0],
                             [-np.sin(gonpitch), 0, np.cos(gonpitch)]])
    ...
    def ewald_cond(phi):
        ...
        return (t_E[0, 0] ** 2 -
                ((Gmat_0[0, 0] * v5[0, 0] +
                  Gmat_0[0, 1] * v5[1, 0]) * np.cos(phi) +
                 (Gmat_0[0, 1] * v5[0, 0] -
                  Gmat_0[0, 0] * v5[1, 0]) * np.sin(phi) +
                 Gmat_0[0, 2] * v5[2, 0] + t_E[0, 0]) ** 2 -
                ((Gmat_0[1, 0] * v5[0, 0] +
                  Gmat_0[1, 1] * v5[1, 0]) * np.cos(phi) +
                 (Gmat_0[1, 1] * v5[0, 0] -
                  Gmat_0[1, 0] * v5[1, 0]) * np.sin(phi) +
                 Gmat_0[1, 2] * v5[2, 0]) ** 2 -
                ((Gmat_0[2, 0] * v5[0, 0] +
                  Gmat_0[2, 1] * v5[1, 0]) * np.cos(phi) +
                 (Gmat_0[2, 1] * v5[0, 0] -
                  Gmat_0[2, 0] * v5[1, 0]) * np.sin(phi) +
                 Gmat_0[2, 2] * v5[2, 0]) ** 2)
    ...
    def get_ij(v5, phi):
        ...
        v1 = 1./0.172*np.dot(np.asarray(detector_info.Omat),
                             v2)+np.asarray(detector_info.Cvec)
        return v1[0, 0], v1[1, 0]
    if (H == 0 and K == 0 and L == 0):
        peaks_detector = np.zeros((0, 3), dtype='float')
    elif(optimize.fsolve(ewald_cond, 45, full_output=1)[2] == 1):

        phi_solutions = np.unique(
            np.around(
                np.array(
                    [optimize.fsolve(ewald_cond, 15) % 360, optimize.fsolve(
                        ewald_cond, 45) % 360, optimize.fsolve(
                        ewald_cond, 75) % 360, optimize.fsolve(
                        ewald_cond, 105) % 360, optimize.fsolve(
                        ewald_cond, 135) % 360, optimize.fsolve(
                        ewald_cond, 165) % 360, optimize.fsolve(
                        ewald_cond, 195) % 360, optimize.fsolve(
                        ewald_cond, 225) % 360, optimize.fsolve(
                        ewald_cond, 255) % 360, optimize.fsolve(
                        ewald_cond, 285) % 360, optimize.fsolve(
                        ewald_cond, 315) % 360, optimize.fsolve(
                        ewald_cond, 345) % 360, optimize.fsolve(
                        ewald_cond, 375) % 360, ]),
                decimals=4))
        # print(phi_solutions)
...
# discard solutions that are not on the detector
    peaks_detector = peaks_detector[np.logical_not(
        np.logical_or(
            peaks_detector[:, 0] > 1474,
            peaks_detector[:, 0] < 1))]
    peaks_detector = peaks_detector[np.logical_not(
        np.logical_or(
            peaks_detector[:, 1] > 1678,
            peaks_detector[:, 1] < 1))]
    if verbose is True:
        print('Check 2:', peaks_detector)
# attempt to realign peaks with a linear correction
    for i in range(0, len(peaks_detector[:, 0])):
        ...
        zvals = input_entry.data.data[zmin:zmax,
                                      ymin:ymax, xmin:xmax].sum(1).sum(1)
        try:
    ...
    peaks_detector = peaks_detector[np.logical_not(
        np.logical_or(
            peaks_detector[:, 2] > 3648,
            peaks_detector[:, 2] < 0))]
    if verbose is True:
        print('Check 4:', peaks_detector)
# three possibilities: no solutions, one solution on detector, or
# two solutions on detector (add error check for more solutions?)
    if(peaks_detector.shape[0] == 0):
        return peaks_detector.astype('float')
    elif(peaks_detector.shape[0] == 1):
        if peaks_detector[0, 2] < 48:
            peaks_detector = np.vstack(
                (peaks_detector, peaks_detector[0, :]+np.array([0, 0, 3600])))
        ...
        if peaks_detector[1, 2] < 48:
            peaks_detector = np.vstack(
                (peaks_detector, peaks_detector[1, :]+np.array([0, 0, 3600])))
...
def generate_xyz_peak_list(
        input_entry, hmin, hmax, kmin, kmax, lmin, lmax, verbose=False):
            ...
            for H in np.arange(hmin, hmax+1):
                ...
                for i in range(0, len(peak_xyz[:, 0])):
                    ...
                    frame_intensity = (
                        ...
                        [zframe, ymin: ymax, xmin: xmax].sum().nxdata +
                        len(
                            np.where(
                                input_entry.data.data
                                [zframe, ymin: ymax, xmin: xmax].nxdata == -1)
                            [0])) * 21. * 21. / (
                        1. *
                        len(
                            np.where(
                    ...
                    xyz_entry = np.array(
                        [[peak_xyz[i, 0],
                          ...
                          peak_xyz[i, 2],
                          input_entry.data.data
                          [zframe, np.int(np.rint(peak_xyz[i, 1])),
                           np.int(np.rint(peak_xyz[i, 0]))].nxdata, H, K, L,
...
def write_inferred_peaks(input_entry, xyz_array):
    ...
    input_entry.peaks_inferred.pixel_count = NXfield(
        xyz_array[:, 3], name='pixel_count')
    input_entry.peaks_inferred.frame_intensity = NXfield(
        xyz_array[:, 7], name='frame_intensity')
...
def get_hkl_element(input_array, H, K, L):
    ...
    elements = np.intersect1d(
        np.where(input_array[:, 4] == H)[0],
        np.intersect1d(
            np.where(input_array[:, 5] == K)[0],
            np.intersect1d(
                np.where(input_array[:, 6] == L)[0],
                np.where(input_array[:, 3] > -10000)[0])))
    for i in range(0, len(elements)):
        xyz_array = np.concatenate(
            (xyz_array, input_array[elements[i:i+1]]), axis=0)
    return xyz_array
...
def determine_mask_frames(input_entry, xval, yval, zval):
    ...
    if ymin < 0:
        ymin = 0
    intensity_list = (input_entry.data.data[
        z+index_list[0]:z+index_list[-1]+1,
        ymin:ymax, xmin:xmax].sum(1).sum(1).nxdata*21.*21. /
        (len(np.where(input_entry.data.data[z, ymin:ymax, xmin:xmax].nxdata
             > -1)[0])))
    mask_coords = np.zeros((0, 4), dtype='int')
    frame_threshold = frame_mask_size(0.0, find_zero=True)
    mask_frames = (np.where(intensity_list > frame_threshold)
                   [0]).astype('int')  # fix this limit!
    for i in range(0, len(mask_frames)):
        new_coords = np.array(
            [[x, y, z + index_list[mask_frames[i]],
              frame_mask_size(intensity_list[mask_frames[i]])]])
...
def make_peaks_inferred_array(input_entry):
    peaks_inferred_array = np.stack((
        input_entry.peaks_inferred.x.nxdata,
        ...
        input_entry.peaks_inferred.radius.nxdata), axis=1)
...
def make_mask_array(input_root):
    ...
    for i in range(0, len(f1_array[:, 0])):
        ...
        if f1_array[i, 3] > 0:
            xyz_array = np.concatenate(
                (xyz_array, determine_mask_frames(
                    input_root.f1, xval, yval, zval)), axis=0)
        elif f1_array[i, 3] < 0:
            hval = f1_array[i, 4]
            kval = f1_array[i, 5]
            lval = f1_array[i, 6]
            ...
            for i in range(0, checka.shape[0]):
                check_mask = determine_mask_frames(
                    input_root.f2, checka[i, 0],
                    checka[i, 1],
                    checka[i, 2])
            ...
            z_intensity = (input_root.f1.data.data[
                zmin:zmax, ymin:ymax, xmin:xmax].sum(1).sum(1).nxdata)
            zval = zframes[np.where(z_intensity == np.amax(z_intensity))[0][0]]
            infer_xyz = np.zeros((width, 4))
            infer_xyz[:, 0] = xval
            infer_xyz[:, 1] = yval
            infer_xyz[:, 2] = np.arange(
                np.rint(zval)-width//2, np.rint(zval)+width//2+1)
            infer_xyz[:, 3] = radius
            if(radius > 0):
    ...
    for i in range(0, len(f2_array[:, 0])):
        ...
        if f2_array[i, 3] > 0:
            xyz_array = np.concatenate(
                (xyz_array,
                 determine_mask_frames(input_root.f2, xval, yval, zval)),
                axis=0)
        elif f2_array[i, 3] < 0:
            hval = f2_array[i, 4]
            kval = f2_array[i, 5]
            ...
            z_intensity = (input_root.f2.data.data[
                zmin:zmax, ymin:ymax, xmin:xmax].sum(1).sum(1).nxdata)
            zval = zframes[np.where(z_intensity == np.amax(z_intensity))[0][0]]
            infer_xyz = np.zeros((width, 4))
            infer_xyz[:, 0] = xval
    ...
    for i in range(0, len(f3_array[:, 0])):
        ...
        if f3_array[i, 3] > 0:
            xyz_array = np.concatenate(
                (xyz_array,
                 determine_mask_frames(input_root.f3, xval, yval, zval)),
                axis=0)
        elif f3_array[i, 3] < 0:
            hval = f3_array[i, 4]
            kval = f3_array[i, 5]
            ...
            z_intensity = (input_root.f3.data.data[
                zmin:zmax, ymin:ymax, xmin:xmax].sum(1).sum(1).nxdata)
            zval = zframes[np.where(z_intensity == np.amax(z_intensity))[0][0]]
            infer_xyz = np.zeros((width, 4))
            infer_xyz[:, 0] = xval
...
def mask_scan(input_root, mask_edges=True, print_time=False):
    ...
    if print_time is True:
        f1_start = time.time()
    write_inferred_peaks(input_root.f1, generate_xyz_peak_list(
        input_root.f1, hmin, hmax, kmin, kmax, lmin, lmax))
    if print_time is True:
        f2_start = time.time()
    write_inferred_peaks(input_root.f2, generate_xyz_peak_list(
        input_root.f2, hmin, hmax, kmin, kmax, lmin, lmax))
    if print_time is True:
        f3_start = time.time()
    write_inferred_peaks(input_root.f3, generate_xyz_peak_list(
        input_root.f3, hmin, hmax, kmin, kmax, lmin, lmax))
    if print_time is True:
        peaks_end = time.time()
    ...
    if print_time is True:
        ...
        print(
            str(mask_xyz_end - peaks_end) +
            ' seconds for determining mask_xyz for all entries')
...
def edge_mask(plane, intensity_factor=0.99, intensity_constant=0.0,
              window_size=5, horiz_size_1=10, horiz_threshold_1=0.8,
              vertical_size=70, vertical_threshold=0.95, horiz_size_2=501,
              horiz_threshold_2=0.3):
    ...
    plane = np.concatenate(
        (plane[3600 - overlap_size: 3600, :],
         plane, plane[0: overlap_size]),
        axis=0)
    plane[plane < 0] = 1
    plane_smoothed = signal.convolve(plane, kern, mode='same')
    plane_mask = (plane[overlap_size:3650+overlap_size, :] -
                  plane_smoothed[(overlap_size)//2:3650+(overlap_size)//2, :] *
                  intensity_factor+intensity_constant)
...
def plane_steps(plane):
    ...
    plane_diff = (plane[:, 0:length-1].astype('int') -
                  plane[:, 1:length].astype('int'))
    return plane_diff
...
def mask_frame_row(row, frame_number, verbose=False):
    ...
    if row.sum() in np.array([-1, 0, 1]):
        if neg1.shape[0] == 0 and pos1.shape[0] == 0:
            ...
            infer_circ = np.array(
                [[(neg1[0]+axis_max)/2, frame_number, (axis_max-neg1[0])/2]])
            circ_array = np.concatenate((circ_array, infer_circ), axis=0)
            return circ_array
            ...
            infer_circ = np.array(
                [[(neg1[0]+pos1[0])/2, frame_number, (pos1[0]-neg1[0])/2]])
            circ_array = np.concatenate((circ_array, infer_circ), axis=0)
            return circ_array
            ...
            for i in range(0, pos1.shape[0]):
                infer_circ = np.array(
                    [[(neg1[i]+pos1[i])/2, frame_number, (pos1[i]-neg1[i])/2]])
                circ_array = np.concatenate((circ_array, infer_circ), axis=0)
            infer_circ = np.array(
                [[(neg1[::-1][0]+axis_max)/2, frame_number,
                 (axis_max-neg1[::-1][0])/2]])
            circ_array = np.concatenate((circ_array, infer_circ), axis=0)
            return circ_array
        elif neg1.shape[0] < pos1.shape[0]:
            ...
            for i in range(0, neg1.shape[0]):
                infer_circ = np.array(
                    [[(neg1[i]+pos1[i+1])/2, frame_number,
                     (pos1[i+1]-neg1[i])/2]])
                circ_array = np.concatenate((circ_array, infer_circ), axis=0)
            infer_circ = np.array([[pos1[0]/2, frame_number, pos1[0]/2]])
            circ_array = np.concatenate((circ_array, infer_circ), axis=0)
...
def mask_edges(input_entry):
    ...
    x_min_plane_mask = np.logical_or(
        edge_mask(
            x_min_plane, intensity_factor=0.99,
            intensity_constant=0.0, window_size=3,
            horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
            vertical_threshold=0.95,
            horiz_size_2=501,
            horiz_threshold_2=0.3),
        edge_mask(
            x_min_plane, intensity_factor=1.5,
            intensity_constant=0.0,
            window_size=1001, horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
            vertical_threshold=0.95,
            horiz_size_2=501,
    ...
    x_max_plane_mask = np.logical_or(
        edge_mask(
            x_max_plane, intensity_factor=0.99,
            intensity_constant=0.0, window_size=3,
            horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
            ...
            horiz_threshold_2=0.3),
        edge_mask(
            x_max_plane, intensity_factor=1.5,
            intensity_constant=0.0,
            window_size=1001, horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
    ...
    y_min_plane_mask = np.logical_or(
        edge_mask(
            y_min_plane, intensity_factor=0.99,
            intensity_constant=0.0, window_size=3,
            horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
            ...
            horiz_threshold_2=0.3),
        edge_mask(
            y_min_plane, intensity_factor=1.5,
            intensity_constant=0.0,
            window_size=1001, horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
    ...
    y_max_plane_mask = np.logical_or(
        edge_mask(
            y_max_plane, intensity_factor=0.99,
            intensity_constant=0.0, window_size=3,
            horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
            ...
            horiz_threshold_2=0.3),
        edge_mask(
            y_max_plane, intensity_factor=1.5,
            intensity_constant=0.0,
            window_size=1001, horiz_size_1=10,
            horiz_threshold_1=0.8,
            vertical_size=70,
    ...
    for i in range(0, number_of_frames):
        x_min_circ = np.concatenate((x_min_circ, mask_frame_row(
            x_min_plane_steps[i], i, verbose=False)), axis=0)

    for i in range(0, number_of_frames):
        x_max_circ = np.concatenate((x_max_circ, mask_frame_row(
            x_max_plane_steps[i], i, verbose=False)), axis=0)

    for i in range(0, number_of_frames):
        y_min_circ = np.concatenate((y_min_circ, mask_frame_row(
            y_min_plane_steps[i], i, verbose=False)), axis=0)

    for i in range(0, number_of_frames):
        y_max_circ = np.concatenate((y_max_circ, mask_frame_row(
            y_max_plane_steps[i], i, verbose=False)), axis=0)
...
def add_edge_xyz(input_entry):
    ...
    xyz_array = np.concatenate(
        (mask_xyz_volume, mask_edges(input_entry)), axis=0)
...
def add_mask_xyz_edges(input_root):
    ...
    entry['mask_xyz_edges']['H'] = np.zeros(
        fx_mask_xyz_edges_array[:, 0].shape[0])
    entry['mask_xyz_edges']['K'] = np.zeros(
        fx_mask_xyz_edges_array[:, 0].shape[0])
    entry['mask_xyz_edges']['L'] = np.zeros(
        fx_mask_xyz_edges_array[:, 0].shape[0])
    entry['mask_xyz_edges']['pixel_count'] = np.zeros(
        fx_mask_xyz_edges_array[:, 0].shape[0])
...
def add_mask_adhoc(input_entry, x, y, z, radius, H, K, L, pixel_count):
    ...
    pixcoll = np.append(entry['mask_xyz_adhoc']
                        ['pixel_count'].nxdata, pixel_count)
...
def update_mask_xyz(input_root):
    ...
    entry['mask_xyz']['radius'] = (
        input_root[scan_entry]['mask_xyz']['radius'].nxdata)
    ...
    entry['mask_xyz']['pixel_count'] = np.zeros(
        entry['mask_xyz']['x'].nxdata.shape[0])
